...
main_fn = loop_collect(mlp.forward, steps, angle_nd)
...
coords = np.zeros((n_points, steps))
batch = points.reshape(2, -1, 512)
coords[:] = f(batch).reshape(n_points, steps)


#### PLOT
fig, ax = plt.subplots(figsize=(20, 20))
c = np.linspace(0,1, steps - 1)
...
out_path = Path("/code/output/phase_plots/")
